...
exports.getPartenaires = async (req, res) => {
  ...
  try {
    ...
    const url = `${baseUrl}/${sitename}/partenaire?maxResults=50`;
    ...
    if (response.status === 200) {
      const partenaires = response.data;  // Les données retournées par l'API
      return res.json(partenaires);  // Retourner les données au client
    } else {
      // Si la réponse est incorrecte, retourner une erreur
      return res.status(500).json({ error: 'Erreur de récupération des patenaires' });
    }
  } catch (error) {
    // Gestion des erreurs en cas de problème avec la requête
    console.error('Erreur lors de la récupération des patenaires:', error.message);
